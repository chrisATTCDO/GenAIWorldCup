...
DATA_BRICKS_URL = os.environ.get('URL')
...
def get_content(user_query):     
    ...
    response = requests.request("POST", cognitive_search_endpoint, headers=headers, data=payload)  
    # Parse the JSON response  
    response_json = response.json()        
    # Extract the page contents  
    page_contents = [doc['content'] for doc in response_json['value']]    
...
def chat() -> Response:
    ...
    #response = handleUserQuery(user_query, client_id) 
    response = data_bricks_model(user_query, client_id)
...
def refreshSpeechToken() -> None:
    ...
    while True:
        ...
        if speech_private_endpoint:
            ...
            #token = ""
            speech_token = f'aad#{speech_resource_url}#{token}'
        else:
...
def data_bricks_model(user_query: str, client_id: uuid.UUID):
    global client_contexts
    client_context = client_contexts[client_id]
    chat_history = client_context['chat_history'] 

    input_df = pd.DataFrame({
        "query": [user_query],
        "chat_history": [json.dumps(chat_history)],  # Assuming the model expects this format; adjust if necessary
    })

    result = score_model(input_df)
    assistant_reply = result['predictions']['response']
    spoken_sentence = assistant_reply

    chat_history.append({"query": user_query, "answer": assistant_reply})
...
def handleUserQuery(user_query: str, client_id: uuid.UUID):
    ...
    tool_content = ''
    spoken_sentence = ''
    context =  get_content(user_query)
    messages[0]['content'] += f"\nAccording to the chat for your help refer the below context.\nContext: {context}"
